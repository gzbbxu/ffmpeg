...
- AVCodec*avcodec_find_decoder(enum AVCodecID id); 通过id
- AVCode*avcodec_find_decode_by_name(const char * name); 通过字符串 比如：硬解码

### AVCodecContext

AVCodecContext * avcodec_alloc_context3(const AVCodec *codec);

void avcodec_free_context(AVCoecContext ** avctx);
...
int linesize[AV_NUM_DATA_POINTERS];  // 视频表示一行数据的大小 ，音频表示 一个通道数据的大小

比如32位 ， 4个字节， 双通道 ，nb_sample = 1024  那么 linesize = 1024 *4 *2 =8192
...
int saple_rate;uint64_t channel_layout; int channels;

int format;  //AVPixelFormat   AVSampleFormat

### avcodec_send_packet 

int avcodec_send_packet（AVCodecContext * avctx,const AVPacket * avpket）
